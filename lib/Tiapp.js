...
Tiapp.prototype.getAndroidVersionCode = function(code) {
	var manifest = this.doc.getElementsByTagName('manifest')[0]
	var attribs = manifest.attributes;

	for (var i = 0, len = attribs.length; i < len; i++) {
		var item = attribs.item(i);

		if (item.nodeName === 'android:versionCode') {
			return item.nodeValue;
		}
	}
};

Tiapp.prototype.setAndroidVersionCode = function(code) {
	var manifest = this.doc.getElementsByTagName('manifest')[0]
	var attribs = manifest.attributes;
	...
	for (var i = 0, len = attribs.length; i < len; i++) {
		...
		if (item.nodeName === 'android:versionCode') {
			var cleanCode = code.replace(/\D+/g, '');

			item.nodeValue = cleanCode;
			item.value = cleanCode;
		}
	}
};

Tiapp.prototype.getAndroidVersionName = function(code) {
	var manifest = this.doc.getElementsByTagName('manifest')[0]
	var attribs = manifest.attributes;
	...
	for (var i = 0, len = attribs.length; i < len; i++) {
		...
		if (item.nodeName === 'android:versionName') {
			return item.nodeValue;
		}
	}
};

Tiapp.prototype.setAndroidVersionName = function(name) {
	var manifest = this.doc.getElementsByTagName('manifest')[0]
	var attribs = manifest.attributes;
